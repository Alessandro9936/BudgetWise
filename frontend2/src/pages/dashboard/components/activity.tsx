...
const ActivityCard = ({ transaction }: { transaction: ITransaction }) => {
  return (
    <div
      key={transaction._id}
      className="relative flex animate-appear flex-col gap-1 rounded-lg p-4 shadow-[0_0_8px_rgba(0,0,0,0.1)]"
    >
...
const Activity = ({ gridDisposition }: { gridDisposition: string }) => {
  ...
  return (
    <section className={`${gridDisposition} flex flex-col gap-y-3`}>
      ...
      <Card classNames="flex-1 p-2 flex flex-col gap-6 lg:overflow-hidden ">
        <div className="grid flex-1 scroll-m-7 auto-rows-min gap-6 overflow-y-auto px-4 pt-4 pb-1 md:grid-cols-autoFill">
